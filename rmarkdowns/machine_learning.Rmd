...
```{r}
rf_freshwat <- randomForest(normalized_score_sdg6_freshwat ~ .,
...
```{r}
importance_df <- as.data.frame(rf_freshwat$importance)
```
...
```{r}
pdp::partial(rf_freshwat, pred.var = "normalized_score_sdg6_wastewat", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_freshwat, pred.var = c("normalized_score_sdg6_wastewat", "normalized_score_sdg3_u5mort"))
# Default PDP
plotPartial(pd)
```
...
Hypothesis:
More developed countries are in cluster 2, less developed countries in cluster 1, and in cluster 3 we see African nations in more state of poverty
