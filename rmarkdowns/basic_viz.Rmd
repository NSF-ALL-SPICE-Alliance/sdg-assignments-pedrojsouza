...
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_2_score, color = goal_10_score)) +
  geom_point()
```

Interpretation:

I chose the goal scores 1, 2 and 10, which are no poverty, zero hunger, and reduced inequality. We see in the chart that the combining factors of no poverty, zero hunger and reduced inequality seem to cluster on the top right, in the lighter blue color, meaning that there is some pattern to be seen regarding these three SDG going hand to hand together. However we can also see a lot of random scatter around the chart, meaning that a lot of countries reach somewhat a no hunger status, but continue in poverty. 
...
```{r}
ggplot(lac_sdr_data %>%
         filter(regions_used_for_the_sdr == "MENA"), aes(x = goal_5_score, y = country)) +
  geom_bar(stat = "identity")
```
...
Interpretation challenge 3:

I chose the sdg score Affordable and Clean Energy for the visualization. We can see above that a little over half of the countries we have data from sit somewhat lower in the count, with almost a quarter chunck having higher counts for affordable and clean energy, which translates to the conclusion that a little more effort is needed in reaching this goal from a lot of countries
