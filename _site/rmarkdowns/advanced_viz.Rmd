...
```{r}
wor1ld <- ne_countries(scale = "medium", returnclass = "sf")
```
...
```{r}
sdr_data_wor1ld_joined <- left_join(sdr_data, wor1ld, by = "iso_a3")
```
...
```{r}
sdr_data_wor1ld_joined <- st_as_sf(sdr_data_wor1ld_joined)
```
...
```{r}
sdr_data_wor1ld_joined <- st_transform(sdr_data_wor1ld_joined, "+proj=longlat +datum=WGS84")
```
...
```{r}
mytext <- paste(
    "Country: ", sdr_data_wor1ld_joined$country,"<br/>", 
    "Goal 7 Score: ", round(sdr_data_wor1ld_joined$goal_7_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)

leaflet(sdr_data_wor1ld_joined) %>% 
  addTiles()  %>% 
...
leaflet(sdr_data_wor1ld_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("PuBu", goal_1_score)(goal_2_score), label = mytext)
```
